...
export default function ChatbotIntegrationPage() {
  ...
  const { data, isLoading, mutate } = useSWR<{ channels: ChannelConfig[]; webhooks: WebhookItem[] }>(
    `/api/master-admin/companies/${companyId}/chatbots/${chatbotId}/integrations`,
    fetcher,
  ...
  const handleToggleChannel = async (channelType: string, enabled: boolean) => {
    ...
    try {
      const response = await fetch(
        `/api/master-admin/companies/${companyId}/chatbots/${chatbotId}/integrations/channels`,
        {
          method: "POST",
  ...
  const handleCreateWebhook = async () => {
    ...
    try {
      const response = await fetch(
        `/api/master-admin/companies/${companyId}/chatbots/${chatbotId}/integrations/webhooks`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
                    ...
                    {isEnabled && (
                      <div className="mt-4 pt-4 border-t border-default-200 flex gap-2">
                        {channel.hasWidgetCustomization ? (
                          <Link href={`/admin/companies/${companyId}/chatbots/${chatbotId}/widget`}>
                            <Button
                              size="sm"
                              variant="secondary"
                              leftIcon={Palette}
