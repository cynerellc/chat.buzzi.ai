...
import {
  ...
  useCompanySessions,
  useRevokeCompanySession,
  useRevokeAllCompanySessions,
} from "@/hooks/master-admin/useCompanySettings";
...
export default function CompanySettingsPage() {
  ...
  const { settings, isLoading, mutate } = useCompanySettings(companyId);
  const { updateSettings, isUpdating } = useUpdateCompanySettings(companyId);
  const { generateKey, isGenerating } = useGenerateCompanyApiKey(companyId);
  const { revokeKey, isRevoking } = useRevokeCompanyApiKey(companyId);
  const { sessions, isLoading: sessionsLoading, mutate: mutateSessions } = useCompanySessions(companyId);
  const { revokeSession, isRevoking: isRevokingSession } = useRevokeCompanySession(companyId);
  const { revokeAllSessions, isRevoking: isRevokingAll } = useRevokeAllCompanySessions(companyId);

  const apiKeyModal = useDisclosure();
  const revokeAllSessionsModal = useDisclosure();
  const revokeApiKeyModal = useDisclosure();
  ...
  const handleRevokeAllDeviceSessions = async () => {
    try {
      ...
      addToast({
        ...
        description: "All company devices have been signed out",
        color: "success",
      });
  ...
  const handleGenerateApiKey = async () => {
    try {
      ...
      addToast({
        ...
        description: "Save the API key now - it won't be shown again",
        color: "warning",
      });
      mutate();
    } catch (error) {
  ...
  const handleRevokeApiKey = async () => {
    try {
      ...
      addToast({
        ...
        description: "The company API key has been revoked",
        color: "success",
      });
      revokeApiKeyModal.onClose();
      mutate();
    } catch (error) {
      addToast({
        title: "Error",
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage settings for {company?.name}</p>
          </div>
        </div>
        ...
        <Card>
          ...
          <CardBody className="space-y-4">
            ...
            <Input
              ...
              description="Custom domain for the company support portal"
            />
    ...
    },
    {
      ...
      content: (
        <Card>
          ...
          <CardBody className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Manage API keys to authenticate requests to the Buzzi API for this company.
            </p>
            {settings?.hasApiKey ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2 p-4 bg-success-50 rounded-lg">
                  ...
                  <span className="text-sm text-success-700">
                    API key is configured (prefix: {settings.apiKeyPrefix}...)
                  </span>
                </div>
                {newApiKey && (
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    leftIcon={Trash2}
                    onClick={revokeApiKeyModal.onOpen}
                  >
                    Revoke Key
                  </Button>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 p-4 bg-muted rounded-lg">
                  ...
                  <span className="text-sm">No API key configured for this company</span>
                </div>
                <Button
                  color="primary"
                  leftIcon={Key}
          ...
          <CardBody>
            {sessionsLoading ? (
              ...
              <p className="text-muted-foreground text-center py-8">
                No active sessions found for this company
              </p>
            )}
          </CardBody>
        </Card>
  ...
  return (
    <div className="space-y-6">
            ...
            {settings?.hasApiKey && (
              <div className="flex items-start gap-3 p-3 bg-warning-50 rounded-lg mb-4">
                ...
                <p className="text-sm text-warning-700">
                  Regenerating will invalidate the current API key. Any applications using
      ...
      <Modal isOpen={revokeApiKeyModal.isOpen} onClose={revokeApiKeyModal.onClose}>
        <ModalContent>
          ...
          <ModalBody>
            ...
            <p>Are you sure you want to revoke the API key for this company?</p>
          </ModalBody>
          <ModalFooter>
            <Button variant="ghost" onClick={revokeApiKeyModal.onClose}>
              Cancel
            </Button>
      ...
      <Modal isOpen={revokeAllSessionsModal.isOpen} onClose={revokeAllSessionsModal.onClose}>
        <ModalContent>
          <ModalHeader className="text-danger">Sign Out All Devices</ModalHeader>
          <ModalBody>
            <div className="flex items-start gap-3 p-3 bg-danger-50 rounded-lg mb-4">
              <AlertTriangle className="w-5 h-5 text-danger flex-shrink-0 mt-0.5" />
              <p className="text-sm text-danger-700">
                This will sign out all users in this company from all their devices.
              </p>
            </div>
            <p>Are you sure you want to sign out all devices?</p>
          </ModalBody>
          <ModalFooter>
