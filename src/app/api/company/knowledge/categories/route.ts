...
export async function GET() {
  try {
    ...
    const sourceCounts = await db
      ...
      .where(eq(knowledgeSources.companyId, company.id))
    ...
    const faqCategoryCounts = await db
      ...
      .from(faqItems)
      .where(eq(faqItems.companyId, company.id))
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        // Also delete any chunks that have this category directly (belt and suspenders)
        try {
          const qdrant = getQdrantService();
          await qdrant.deleteByFilter(COLLECTIONS.KNOWLEDGE_CHUNKS, {
            must: [
              { key: "companyId", match: { value: company.id } },
