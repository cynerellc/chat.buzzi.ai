...
/**
 * GET /api/company/agents/[agentId]/code
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get the agent and verify it belongs to the company and is a custom package
    ...
    if (!agent.packageId) {
      return NextResponse.json(
        { error: "No package associated with this agent" },
        { status: 404 }
      );
    }

    const packageDir = path.join(CHATBOT_PACKAGES_DIR, agent.packageId);
