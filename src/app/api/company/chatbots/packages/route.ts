...
// Variable definition for chatbot packages
...
export async function GET() {
  try {
    ...
    const transformedPackages: ChatbotPackageItem[] = packages.map((pkg) => ({
      id: pkg.id,
      name: pkg.name,
      slug: pkg.slug,
    ...
    // L1: Add cache headers - chatbot packages rarely change
    const response = NextResponse.json({ packages: transformedPackages });
    response.headers.set("Cache-Control", "private, max-age=300, stale-while-revalidate=600");
    return response;
  } catch (error) {
    console.error("Error fetching chatbot packages:", error);
    return NextResponse.json(
      { error: "Failed to fetch chatbot packages" },
      { status: 500 }
    );
  }
}
