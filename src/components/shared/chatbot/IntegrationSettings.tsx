...
const comingSoonIntegrations = [
  { type: "slack", name: "Slack", description: "Send notifications and alerts to Slack channels", icon: "S", color: "bg-purple-100 dark:bg-purple-900/30", textColor: "text-purple-600" },
  { type: "hubspot", name: "HubSpot", description: "Sync conversations and contacts with HubSpot CRM", icon: "H", color: "bg-orange-100 dark:bg-orange-900/30", textColor: "text-orange-600" },
  { type: "salesforce", name: "Salesforce", description: "Integrate with Salesforce CRM", icon: "SF", color: "bg-blue-100 dark:bg-blue-900/30", textColor: "text-blue-600" },
  { type: "zendesk", name: "Zendesk", description: "Escalate tickets to Zendesk Support", icon: "Z", color: "bg-green-100 dark:bg-green-900/30", textColor: "text-green-600" },
];
...
export function IntegrationSettings({
  ...
  const handleToggleChannel = async (channelType: string, enabled: boolean) => {
    ...
    try {
      const response = await fetch(channelsApiUrl, {
        ...
        body: JSON.stringify({ channel: channelType, isActive: enabled }),
      });
  ...
  const handleSaveChannelSettings = async () => {
    ...
    try {
      const response = await fetch(channelsApiUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ channel: selectedChannel.type, credentials: channelCredentials }),
      });
  ...
  const copyToClipboard = (text: string, label: string) => {
    ...
    addToast({ title: "Copied", description: `${label} copied to clipboard`, color: "success" });
  };
  ...
  const handleCreateWebhook = async () => {
    ...
    setIsCreating(true);
    try {
      const response = await fetch(webhooksApiUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      addToast({ title: "Webhook Created", description: "Your webhook has been created successfully", color: "success" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Integration</h2>
          <p className="text-sm text-muted-foreground">Configure channels and integrations for {chatbotName}</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <p className="text-sm text-muted-foreground">Enable channels to receive messages from different platforms</p>
          </div>
        </CardHeader>
        <CardBody>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isEnabled && <Chip color="success" size="sm">Active</Chip>}
                          </div>
                          <p className="text-sm text-muted-foreground mt-1">{channel.description}</p>
                          {config?.lastConnectedAt && (
                            <p className="text-xs text-muted-foreground mt-2">Last active: {formatDate(config.lastConnectedAt)}</p>
                          )}
                          {config?.lastError && <p className="text-xs text-danger mt-1">Error: {config.lastError}</p>}
                        </div>
                      </div>
                      <Switch
                    ...
                    {isEnabled && (
                      <div className="mt-4 pt-4 border-t border-default-200 flex gap-2">
                        {channel.hasWidgetCustomization ? (
                          <Link href={widgetUrl}>
                            <Button size="sm" variant="secondary" leftIcon={Palette}>Widget Customization</Button>
                          </Link>
                        ) : (
                          <Button size="sm" variant="secondary" leftIcon={Settings2} onPress={() => handleOpenChannelSettings(channel)}>Settings</Button>
                        )}
                      </div>
                    )}
                  </CardBody>
                ...
                <CardBody className="p-4">
                  <div className="flex flex-col items-center text-center">
                    <div className={`w-12 h-12 rounded-lg ${integration.color} flex items-center justify-center mb-3`}>
                      <span className={`text-lg font-bold ${integration.textColor}`}>{integration.icon}</span>
                    </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                Build custom integrations using our REST API. Access conversations, messages, and more programmatically.
              </p>
      ...
      <Card>
        <CardHeader className="flex justify-between items-center">
          ...
          <Button color="primary" size="sm" leftIcon={Plus} onPress={webhookModal.onOpen}>Add Webhook</Button>
        </CardHeader>
        <CardBody>
          {webhooks.length > 0 ? (
            <TableRoot>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {webhook.events.slice(0, 2).map((event) => (
                          <Chip key={event} size="sm">{event.split(".")[1]}</Chip>
                        ))}
                        {webhook.events.length > 2 && <Chip size="sm">+{webhook.events.length - 2}</Chip>}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-3 text-xs">
                        <span className="flex items-center gap-1 text-success">
                          <CheckCircle className="w-3 h-3" />{webhook.successfulDeliveries}
                        </span>
                        <span className="flex items-center gap-1 text-danger">
                          <XCircle className="w-3 h-3" />{webhook.failedDeliveries}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell><Switch isSelected={webhook.isActive} size="sm" /></TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        <Button size="icon" variant="ghost" title="Test webhook"><RefreshCw className="w-4 h-4" /></Button>
            ...
            <div className="text-center py-8">
              ...
              <Button variant="secondary" onClick={webhookModal.onOpen}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <div className="space-y-4">
              <Input label="Name" placeholder="My Webhook" value={webhookForm.name} onValueChange={(value) => setWebhookForm((prev) => ({ ...prev, name: value }))} isRequired />
              <Textarea label="Description" placeholder="Optional description" value={webhookForm.description} onValueChange={(value) => setWebhookForm((prev) => ({ ...prev, description: value }))} />
              <Input label="Endpoint URL" placeholder="https://api.example.com/webhook" value={webhookForm.url} onValueChange={(value) => setWebhookForm((prev) => ({ ...prev, url: value }))} isRequired startContent={<Globe className="w-4 h-4 text-muted-foreground" />} />
              <div>
                <label className="text-sm font-medium mb-2 block">Events to Subscribe <span className="text-danger">*</span></label>
                <div className="space-y-2">
                  {webhookEvents.map((event) => (
                    <div key={event.key} className="flex items-center justify-between p-3 border border-default-200 rounded-lg">
                      <div>
                      ...
                      <Switch
                        ...
                        onValueChange={(checked) => {
                          setWebhookForm((prev) => ({
                            ...prev,
                            events: checked ? [...prev.events, event.key] : prev.events.filter((e) => e !== event.key),
                          }));
                        }}
                      />
                    </div>
              ...
              <Input label="Secret (Optional)" placeholder="Used to verify webhook signatures" value={webhookForm.secret} onValueChange={(value) => setWebhookForm((prev) => ({ ...prev, secret: value }))} description="We'll include this in the X-Webhook-Signature header" />
            </div>
          </ModalBody>
          <ModalFooter>
            <Button variant="ghost" onPress={webhookModal.onClose}>Cancel</Button>
            <Button color="primary" isLoading={isCreating} disabled={!webhookForm.name || !webhookForm.url || webhookForm.events.length === 0} onPress={handleCreateWebhook}>Create Webhook</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
      ...
      <Modal isOpen={channelSettingsModal.isOpen} onClose={channelSettingsModal.onClose} size="lg">
        <ModalContent>
              ...
              {selectedChannel && (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground font-normal">Configure your {selectedChannel.name} integration</p>
                  </div>
                </>
              )}
            </div>
              ...
              <div className="space-y-6">
                {getChannelConfig(selectedChannel.type)?.webhookUrl && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground mb-2">Add this URL to your {selectedChannel.name} developer settings to receive messages</p>
                    <div className="flex gap-2">
                      <Input value={getChannelConfig(selectedChannel.type)?.webhookUrl || ""} readOnly className="font-mono text-sm" />
                      <Button size="icon" variant="secondary" onPress={() => copyToClipboard(getChannelConfig(selectedChannel.type)?.webhookUrl || "", "Webhook URL")}>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">Enter your {selectedChannel.name} API credentials. These are securely stored and encrypted.</p>
                  ...
                  {selectedChannel.credentialFields?.map((field) => (
                    <div key={field.field}>
                      <Input
                        ...
                        endContent={
                          field.type === "password" && (
                            <button type="button" className="text-muted-foreground hover:text-foreground" onClick={() => setShowSecrets((prev) => ({ ...prev, [field.field]: !prev[field.field] }))}>
                              {showSecrets[field.field] ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                            </button>
                          )
                        }
                      />
          ...
          <ModalFooter>
            <Button variant="ghost" onPress={channelSettingsModal.onClose}>Cancel</Button>
            <Button color="primary" isLoading={savingSettings} onPress={handleSaveChannelSettings}>Save Settings</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </div>
