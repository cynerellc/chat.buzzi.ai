...
// Mutation helpers
async function createCategoryMutation(
  url: string,
  { arg }: { arg: { name: string } }
) {
...
async function reindexAllMutation(url: string) {
...
export function KnowledgeBasePage({
  ...
  // Build URLs
  const categoriesUrl = `${baseApiUrl}/categories`;
  const faqUrl = `${baseApiUrl}/faq`;
  const reindexUrl = `${baseApiUrl}/reindex`;
  ...
  const handleConfirmDeleteSource = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`${baseApiUrl}/${deleteSourceModal.sourceId}`, {
        method: "DELETE",
      });
      if (response.ok) {
        ...
        <div className="space-y-4">
          ...
          {categories.map((category) => (
            <CategorySection
              ...
              onAddSource={() => onAddSource(category.name)}
              onAddFaq={() => onAddFaq(category.name)}
